proc_cls proc near      ; Процедура очистки экрана и установки цветов фона и текста
    ...
    mov dx, 204Fh       ; dh = 18h - строка нижнего правого угла
...
proc_set_cursor proc near ; ВМЕСТО ПРОЦЕДУРЫ, ИСПОЛЬЗУЙТЕ МАКРОС __НИЖЕ__
push ax
push bx
mov  bh, 0
mov  ah, 2h
...
proc_print_int proc near    ; ВМЕСТО ПРОЦЕДУРЫ, ИСПОЛЬЗУЙТЕ МАКРОС __НИЖЕ__
                            ; SI СОДЕРЖИТ СМЕЩЕНИЕ ДЛЯ БУФЕРА, А САМО ЧИСЛО ДЛЯ ВЫВОДА -  В EAX
    mov bx, si
    add bl, byte ptr [si]              ; размер максимального числа, пишем число задом наперед (с конца буфера) + служебные
